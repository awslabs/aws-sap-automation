...
SAP_HANADB_SOFTWARE_S3_BUCKET=$(sed -n 's|.*"SAPINST_CD_RDBMS":"\([^"]*\)".*|\1|p' parameters.json)
SAP_HANACLIENT_SOFTWARE_S3_BUCKET=$(sed -n 's|.*"SAPINST_CD_RDBMS_CLIENT":"\([^"]*\)".*|\1|p' parameters.json)
...
#MASTER_PASSWORD_SECRET=$(aws secretsmanager describe-secret --secret-id $hanamasterpasskey --query 'Name')
